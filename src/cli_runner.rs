use crate::{cli_answers::AnswerProcessor, cli_hooks::HookManager, cli::Args};
use crate::{
    config::Config,
    dialoguer::confirm,
    error::{Error, Result},
...
pub fn run(args: Args) -> Result<()> {
    ...
    // Setup hook manager
    let hook_manager = HookManager::new(engine.as_ref(), args.should_skip_hook_confirms());
    let (pre_hook_filename, post_hook_filename) =
        hook_manager.render_hook_filenames(&config_v1.pre_hook_filename, &config_v1.post_hook_filename)?;

    let (execute_hooks, pre_hook_file, post_hook_file) =
        hook_manager.prepare_hooks(&template_root, &pre_hook_filename, &post_hook_filename)?;
    ...
    let initial_answers = answer_processor.get_initial_answers(args.answers.clone(), pre_hook_stdout)?;
    let answers = answer_processor.collect_answers(&Config::V1(config_v1), initial_answers)?;
...
fn process_template_files(
    ...
    for dir_entry in WalkDir::new(template_root) {
        ...
        match processor.process(&template_entry) {
            Ok(file_operation) => {
                let user_confirmed_overwrite = handle_file_operation(args, &file_operation)?;
...
fn handle_file_operation(args: &Args, file_operation: &TemplateOperation) -> Result<bool> {
